...
def before():
    ...
    response = requests.get(SERVER_URL + '/api/dir/list', data={'path': SERVER_UNIQUE_TEST_DIR_NAME})
    assert response.status_code == 400
    response = requests.post(SERVER_URL + '/api/dir/new', data={'path': '.', 'dirname': SERVER_UNIQUE_TEST_DIR_NAME})
...
def after():
    ...
    # should return 400 and path not found
    response = requests.post(SERVER_URL + '/api/dir/remove', data={'path': SERVER_UNIQUE_TEST_DIR_NAME})
    assert response.status_code == 200
    response = requests.get(SERVER_URL + '/api/dir/list', data={'path': SERVER_UNIQUE_TEST_DIR_NAME})
    assert response.status_code == 400
...
def test_api_upload_download_file():
    ...
    response = requests.get(SERVER_URL + '/api/file/download', data={'path': os.path.join(SERVER_UNIQUE_TEST_DIR_NAME, 'requiments.txt')})
...
def test_api_create_empty_file():
    ...
    response = requests.post(SERVER_URL + '/api/file/new', data={'path': '.', 'filename': filename})
    assert response.status_code == 200
...
def test_api_create_empty_dir():
    ...
    response = requests.post(SERVER_URL + '/api/dir/new', data={'path': '.', 'dirname': dirname})
    assert response.status_code == 200
...
def test_api_file_move():
    ...
    response = requests.post(SERVER_URL + '/api/dir/new', data={'path': SERVER_UNIQUE_TEST_DIR_NAME, 'dirname': 'dir_test'})
    assert response.status_code == 200
    
    # move the file
    response = requests.post(SERVER_URL + '/api/file/move', data={'src': SERVER_UNIQUE_TEST_DIR_NAME, 'dict': dist})
    assert response.status_code == 200
    ...
    response = requests.get(SERVER_URL + '/api/dir/list', data={'path': dist})
    assert response.status_code == 200
    response_json = response.json()
...
def test_api_file_copy():
    ...
    # copy the file
    response = requests.post(SERVER_URL + '/api/file/copy', data={'src': SERVER_UNIQUE_TEST_DIR_NAME, 'dict': dist})
    assert response.status_code == 200
...
def test_api_dir_copy():
    ...
    response = requests.post(SERVER_URL + '/api/dir/new', data={'path': SERVER_UNIQUE_TEST_DIR_NAME, 'dirname': 'dir_test'})
    assert response.status_code == 200
    response = requests.post(SERVER_URL + '/api/dir/new', data={'path': SERVER_UNIQUE_TEST_DIR_NAME, 'dirname': 'dir_test_2'})
    assert response.status_code == 200 
    
    # make sure the both directories exist
    response = requests.post(SERVER_URL + '/api/dir/copy', data={'src': src, 'dist': dist})
    assert response.status_code == 200

    response = requests.get(SERVER_URL + '/api/dir/list', data={'path': src})
    assert response.status_code == 200
    response_json = response.json()
...
def test_api_remove_file():
    ...
    response = requests.post(SERVER_URL + '/api/file/remove', data={'path': os.path.join(SERVER_UNIQUE_TEST_DIR_NAME, 'requiments.txt')})
    assert response.status_code == 200
...
def test_api_remove_dir():
    ...
    response = requests.post(SERVER_URL + '/api/dir/remove', data={'path': path})
    assert response.status_code == 200
